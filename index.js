...
function customSet(arr) {
  ...
  for (const el of setA) {
    if (typeof el == 'number') {setA.delete(el)}
  }
  return setA
...
function filterAndAdd(mySet, arr) {
  for (let a of mySet) {
    if (typeof a == 'number') {mySet.delete(a)}
  }
...
function checkValueAndType(mySet, value) {
  if (mySet.has(value)) {return `Множина має значення "${value}" типу "${typeof value}"`}
...
function setToArray(mySet) {
  let arrA = [... mySet]
  return arrA.filter(el => typeof el == 'string').sort()
...
function getDifference(set1, set2) {
  ...
  for (const a of set1) {
    if (!set2.has(a)) {differenceSet.add(a)}
  }
...
function getIntersection(arr1, arr2) {
  ...
  for (const a of arr1) {
    if (arr2.includes(a)) {intersectionSet.add(a)}
  }
...
function iterateSet(set) {
  ...
  for (const a of setKeys) {console.log(a)}
  for (const a of setValues) {console.log(a)}
  for (const a of setEntries) {console.log(a)}
...
function sumNumbers(set) {
  let sum = 0
  set.forEach(el => {if (typeof el === 'number') {sum = sum + el}})
  return sum
